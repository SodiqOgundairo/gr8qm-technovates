    ...
    <AdminLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <h1 className="text-2xl font-bold text-white">DevignFX Licenses</h1>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            ...
            { label: "Revenue", value: `₦${stats.revenue.toLocaleString()}`, color: "text-skyblue" },
          ].map((s) => (
            <div key={s.label} className="bg-oxford-card border border-oxford-border rounded-lg p-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <input
            ...
            className="flex-1 px-4 py-2 bg-oxford-card border border-oxford-border rounded-lg text-white text-sm placeholder:text-gray-500 focus:outline-none focus:border-skyblue"
          />
              ...
              <button
                ...
                className={`px-3 py-2 rounded-lg text-xs font-medium capitalize transition-colors ${
                  filter === s
                    ...
                    : "bg-oxford-card border border-oxford-border text-gray-400 hover:text-white"
                }`}
              >
                {s}
              </button>
        ...
        {loading ? (
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="text-left text-gray-500 border-b border-oxford-border">
              ...
              <tbody className="text-gray-300">
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-oxford-border/50 hover:bg-oxford-card/50">
                    ...
                    <td className="py-3 relative">
                      ...
                      {actionMenu === l.id && (
                        <div className="absolute right-0 top-full mt-1 bg-oxford-card border border-oxford-border rounded-lg shadow-xl z-50 min-w-[160px]">
                          <button
                            onClick={() => { setEditLicense(l); setShowModal(true); setActionMenu(null); }}
                            className="w-full text-left px-4 py-2 text-sm text-gray-300 hover:bg-oxford-border/50 hover:text-white"
                          >
                            Edit
                          </button>
                          <button
                          ...
                          {l.status === "active" && (
                            <>
                              <button
                                ...
                                className="w-full text-left px-4 py-2 text-sm text-yellow-400 hover:bg-oxford-border/50"
                              >
                                Suspend
                              </button>
                              <button
                                onClick={async () => { await revokeLicense(l.id, false, "Revoked by admin"); setActionMenu(null); }}
                                className="w-full text-left px-4 py-2 text-sm text-red-400 hover:bg-oxford-border/50"
                              >
                                Revoke
                              </button>
                              <button
                          ...
                          {(l.status === "revoked" || l.status === "suspended") && (
                            <button
                              ...
                              className="w-full text-left px-4 py-2 text-sm text-green-400 hover:bg-oxford-border/50"
                            >
                              Reactivate
                            </button>
                          )}
                          {l.machine && (
                            <button
                              ...
                              className="w-full text-left px-4 py-2 text-sm text-orange-400 hover:bg-oxford-border/50"
                            >
                              Unbind Machine
                            </button>
                          )}
                          <button
                            ...
                            className="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-oxford-border/50"
                          >
                            Delete
                          </button>
                        </div>
        ...
        {showLog && (
          <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={() => setShowLog(null)}>
            <div className="bg-oxford-card border border-oxford-border rounded-xl w-full max-w-2xl max-h-[80vh] overflow-hidden" onClick={(e) => e.stopPropagation()}>
              <div className="flex items-center justify-between p-6 border-b border-oxford-border">
                <h2 className="text-lg font-bold text-white">
                  Activation Log — <span className="font-mono text-sm text-gray-400">{showLog}</span>
                </h2>
                <button onClick={() => setShowLog(null)} className="text-gray-400 hover:text-white">&times;</button>
              </div>
                ...
                {logEntries.length === 0 ? (
                  ...
                  <div className="space-y-3">
                    {logEntries.map((entry) => (
                      <div key={entry.id} className="bg-oxford/50 border border-oxford-border/50 rounded-lg p-3">
...
const LicenseModal: React.FC<LicenseModalProps> = ({ license, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div className="bg-oxford-card border border-oxford-border rounded-xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-6 border-b border-oxford-border">
          <h2 className="text-lg font-bold text-white">
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          {!license && (
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm font-mono focus:outline-none focus:border-skyblue"
              />
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.name} onChange={(e) => set("name", e.target.value)} className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Email</label>
              <input type="email" value={form.email} onChange={(e) => set("email", e.target.value)} className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <select value={form.status} onChange={(e) => set("status", e.target.value)} className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue">
            ...
            <div>
              ...
              <select value={form.tier} onChange={(e) => set("tier", e.target.value)} className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="date" value={form.expires} onChange={(e) => set("expires", e.target.value)} className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Max Days</label>
              <input type="number" value={form.max_days} onChange={(e) => set("max_days", e.target.value)} placeholder="∞" className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
            </div>
            <div>
              <label className="block text-xs text-gray-500 mb-1">Profit Cap %</label>
              <input type="number" value={form.max_profit_pct} onChange={(e) => set("max_profit_pct", e.target.value)} placeholder="∞" className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
            </div>
          </div>

          <div>
            <label className="block text-xs text-gray-500 mb-1">Build ID</label>
            <input type="text" value={form.build_id} onChange={(e) => set("build_id", e.target.value)} placeholder="BLD-XXXXXXXX" className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm font-mono focus:outline-none focus:border-skyblue" />
          </div>

          <div>
            <label className="block text-xs text-gray-500 mb-1">Custom Denial Message</label>
            <input type="text" value={form.message} onChange={(e) => set("message", e.target.value)} placeholder="Leave blank for default" className="w-full px-3 py-2 bg-oxford border border-oxford-border rounded-lg text-white text-sm focus:outline-none focus:border-skyblue" />
          </div>
          ...
          <div className="flex gap-3 pt-2">
            <button type="button" onClick={onClose} className="flex-1 px-4 py-2 border border-oxford-border text-gray-400 rounded-lg text-sm hover:text-white transition-colors">
              Cancel
            </button>
            <button type="submit" disabled={saving} className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-500 text-white rounded-lg text-sm font-medium transition-colors disabled:opacity-50">
